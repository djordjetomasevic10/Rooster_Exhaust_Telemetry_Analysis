...
wav_path = 'rooster_May20.wav'   # place the WAV in same folder or edit path
...
for i in range(len(t)):
    ...
    if np.max(col) < 1e-11: 
        ...
        search_mask = (f_sel >= 20) & (f_sel <= 220)
...
# Mathematically verified multiplier for an un-merged twin-turbo V8 exhaust bank (4 pulses/rev) following first sub harmonic
multiplier = 30.0
...
plt.ylim(10,5000)
plt.xlim(1.4, 16.0)     # <-- Add this line here to crop the time axis
...
ax1.set_xlim(1.4, 16.0)  # <-- Add this line here to crop the time axis
...
# Create a mask to only keep data from 1.4 to 16 seconds
valid_time_mask = (t >= 1.4) & (t <= 16.0)
