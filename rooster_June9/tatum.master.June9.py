...
wav_path = 'rooster_June9.wav'   # place the WAV in same folder or edit path
...
for i in range(len(t)):
    ...
    if np.max(col) < 1e-11:
        ...
        search_mask = (f_sel >= 60) & (f_sel <= 220) # Street mask
        ...
        if len(peaks) > 0:
            idx = int(peaks[0]) # Forces it to a single integer
        else:
            idx = int(np.argmax(col_search))
        
 # --- PARABOLIC INTERPOLATION FOR SUB-BIN ACCURACY ---
        # Ensure the peak isn't on the absolute edge of the search array
        if 0 < idx < len(col_search) - 1:
            y1 = col_search[idx - 1]
            y2 = col_search[idx]
            y3 = col_search[idx + 1]
            ...
            denom = (2.0 * y2 - y1 - y3)
            if denom != 0:
                # Corrected formula for peak shift direction
                delta = 0.5 * (y3 - y1) / denom
                # Corrected bin spacing extraction
                bin_spacing = f_search[1] - f_search[0]
                # Interpolate the exact frequency between bins
                dom_freq[i] = f_search[idx] + (delta * bin_spacing)
            else:
                dom_freq[i] = f_search[idx]
        else:
            dom_freq[i] = f_search[idx]           

dom_freq_filtered = medfilt(dom_freq, kernel_size=5) # Must be an odd integer
# Mathematically verified multiplier for an un-merged twin-turbo V8 exhaust bank (4 pulses/rev)
multiplier = 15.0
crank_rpm = dom_freq_filtered * multiplier 
# Convert to a temporary Pandas Series to use its advanced rolling window feature
rpm_series = pd.Series(crank_rpm)
freq_series = pd.Series(dom_freq_filtered) 
# Convert to a temporary Pandas Series to use its advanced rolling window feature
...
crank_rpm_smoothed = rpm_series.rolling(window=15, center=True, win_type='gaussian').mean(std=2).fillna(rpm_series)
dom_freq_smoothed = freq_series.rolling(window=15, center=True, win_type='gaussian').mean(std=2).fillna(freq_series)
...
plt.ylim(10,5000)
plt.xlim(0.55, 33.0)     # <-- Add this line here to crop the time axis
...
ax1.set_xlim(0.55, 33.0)  # <-- Add this line here to crop the time axis
